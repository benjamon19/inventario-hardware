    ...
    <div ref={topRef} className="space-y-4 relative overflow-x-hidden">
      {detalleItem ? (
        <DetalleView
          ...
          onEdit={(item) => openEdit(item as any)}
          onDelete={(item) => { setDeleteItem(item as any); }}
          onMoveStock={handleMoveStock as any}
      ...
      <div className="space-y-4">
        ...
        {loading ? (
          ...
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1 mask-fade-right w-full">
            <button onClick={() => setFilterCategoria('')} className={`rounded-xl px-3 py-2 text-xs font-bold border transition-all cursor-pointer whitespace-nowrap shrink-0 ${!filterCategoria ? 'bg-slate-900 text-white border-slate-900' : 'bg-slate-50 text-slate-600 border-slate-200 hover:border-slate-300'}`}>Todas</button>
            {sortedCategorias.map(cat => (
              <button key={cat.id} onClick={() => setFilterCategoria(filterCategoria === cat.nombre ? '' : cat.nombre)} className={`rounded-xl px-3 py-2 text-xs font-bold border transition-all cursor-pointer whitespace-nowrap shrink-0 ${filterCategoria === cat.nombre ? 'bg-slate-900 text-white border-slate-900' : 'bg-slate-50 text-slate-600 border-slate-200 hover:border-slate-300 hover:text-slate-900'}`}>{cat.nombre}</button>
            ))}
          </div>
        ) : null}
        ...
        {loading ? (
          ...
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1 mask-fade-right px-1 w-full">
            ...
            {sortedEstados.map(est => {
              ...
              return (
                <button key={est.id} onClick={() => setFilterEstado(active ? '' : est.nombre)} className={`flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-bold border transition-all cursor-pointer whitespace-nowrap shrink-0 ${active ? `${badge} ring-2 ring-offset-1 ring-current` : `${badge} opacity-60 hover:opacity-100`}`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${dot}`} />{est.nombre}
                </button>
              );
            })}
            {filterEstado && <button onClick={() => setFilterEstado('')} className="text-[10px] font-bold text-slate-400 hover:text-slate-600 underline cursor-pointer shrink-0 ml-1">Limpiar</button>}
          </div>
        ) : null}
