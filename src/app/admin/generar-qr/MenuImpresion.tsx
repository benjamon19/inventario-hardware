...
export default function MenuImpresion({ isOpen, onClose, onConfirmPrint, selectedCount, selectedItems }: MenuImpresionProps) {
  ...
  // Detectar mobile y pantallas medias
  useEffect(() => {
    const checkSize = () => {
      const w = window.innerWidth;
      setIsMobile(w < 1024);
      setIsMedium(w >= 768 && w <= 1536);
    };
    checkSize();
              ...
              <Dialog.Panel className="relative transform overflow-hidden rounded-3xl bg-white text-left shadow-2xl transition-all w-full max-w-6xl border border-slate-100 flex flex-col md:flex-row">
                ...
                <div className="flex-1 bg-slate-100 p-8 sm:p-12 flex flex-col items-center justify-between relative overflow-hidden">
                  ...
                  <div className="flex-1 flex items-center justify-center w-full my-8">
                    <div className="bg-white shadow-2xl flex flex-row items-center p-[1%] transition-all duration-300 overflow-hidden" style={{ width: `${current.width * (isMedium ? 35 : 60)}px`, height: `${current.height * (isMedium ? 35 : 60)}px`, minWidth: isMedium ? '100px' : '150px', minHeight: isMedium ? '40px' : '60px' }}>
                      ...
                      <div className="flex-1 flex items-center pl-[2%] pr-2 overflow-hidden h-full">
                        <p className="font-black text-slate-900 leading-[1.1]" style={{ fontSize: `${current.fontSize * (isMedium ? 0.9 : 1.5)}px`, whiteSpace: current.wrapText ? 'pre-wrap' : 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{current.text}</p>
                      </div>
                    </div>
                  </div>

                  <div className="w-full max-w-lg bg-white/80 backdrop-blur-md p-6 rounded-4xl border border-slate-200 shadow-sm space-y-3">
