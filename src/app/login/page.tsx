...
export default function LoginPage() {
  ...
  // ── Escala solo en cliente para evitar hydration mismatch ──
  useEffect(() => {
    const applyScale = () => {
      if (!wrapperRef.current) return;
      const w = window.innerWidth;
      // Notebook FHD: 768px–1349px → 80%. Resto: sin escala.
      if (w >= 768 && w < 1350) {
  ...
  return (
    <main className={`flex min-h-screen items-center justify-center ${currentTheme.background} p-6 relative`}>
      ...
      {/* ref para escala JS — sin clases de transformación para evitar hydration mismatch */}
      <div ref={wrapperRef} className="w-full max-w-md relative z-10">
        <div className={`w-full rounded-2xl border ${currentTheme.border} ${currentTheme.card} shadow-xl mt-16 overflow-visible backdrop-blur-sm`}>
